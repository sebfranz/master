...
```{r}
  ...
  std_of_residuals_of_target_genes <-  matrix(0, nrow = n_target_genes, ncol = n_cell_clusters)
  ...
  for(cell_cluster in 1:n_cell_clusters){
    ...
    TARGET_GENE_RESIDUAL_VARIANCE[cell_cluster] <- mean((intercept_and_regulatorgenes %*% models[[cell_cluster]]$coefficients)^2)
    ...
    std_of_residuals_of_target_genes[cell_cluster]  <- var(std_of_residuals_of_target_genes_temp)
    # std_of_residuals_of_target_genes[cell_cluster] <- mean((std_of_residuals_of_target_genes_temp-mean(std_of_residuals_of_target_genes_temp))^2)
  }
  ...
  for(cell in 1:nrow(dat)){
    for(cell_cluster in 1:n_cell_clusters){
      ...
      loglike[cell,cell_cluster] <- SQUARED_ERROR / std_of_residuals_of_target_genes[cell_cluster]
    }
  }
  ...
  p <- ggplot(data = as.tibble(cbind(loglike, true_cell_clust)),
...
for(cell in 1:nrow(dat)){
  ...
  SS_tot[cell]  <- sum((dat[,ind_targetgenes] - target_gene_means[cell])^2)
...
for(cell in 1:nrow(dat)){
  for(cell_cluster in 1:n_cell_clusters){
    ...
    if(any(is.na(models[[cell_cluster]]$coefficients))){
      NA_coeffs <-  unname(which(is.na(models[[cell_cluster]]$coefficients)))
      S_ERR <- (dat[cell,ind_targetgenes] - as.vector(c(1,dat[cell,c(ind_reggenes, -NA_coeffs)])) %*% models[[cell_cluster]]$coefficients[-NA_coeffs])^2
    }
    S_ERR <- (dat[cell,ind_targetgenes] - as.vector(c(1,dat[cell,ind_reggenes])) %*% models[[cell_cluster]]$coefficients)^2
