...
loglike_all <- vector("list", length = max_iter)
...
#plot output
p + labs(color = "Cell cluster")

```

Now that we have fitted some models, it's time to re-allocate cells to appropriate clusters.
To do this, we calculate the probability of each cell to follow from each linear model. This is an e xtremely simplified version of what we will later look into. 
Since there is only one regulator gene, we will skip the L1 penalty for adding coeffitients that we would use in a larger model, and instead only look at the negative loglikelyhood


```{r}
  # For all cells, calculate the likelyhood of coming from the model corresponding to each 
  loglike <- matrix(0, nrow = nrow(dat), ncol = n_cell_clusters)

  #calculate the residual target gene variance for each gene and cluster
    #so just one gene
  TARGET_GENE_RESIDUAL_VARIANCE <-  matrix(0, nrow = n_target_genes, ncol = n_cell_clusters)
  for(cell_cluster in 1:n_cell_clusters){
    TARGET_GENE_RESIDUAL_VARIANCE[cell_cluster] <- mean((as.matrix(cbind(rep(1,nrow(dat)),dat[,-1])) %*% models[[cell_cluster]]$coefficients)^2)
  }
  ...
  for(cell in 1:nrow(dat)){
    for(cell_cluster in 1:n_cell_clusters){
      ...
      SQUARED_ERROR <- (dat[cell,1] - as.vector(c(1,dat[cell,-1])) %*% models[[cell_cluster]]$coefficients)^2
      
      loglike[cell,cell_cluster] <- SQUARED_ERROR / TARGET_GENE_RESIDUAL_VARIANCE[cell_cluster]
    }
  }

  loglike_all[[i_main]] <- loglike

  r2plot(iteration = i_main,
       r2 = loglike,
       prev_cell_clust = true_cell_clust)

  ggplot(data = as.tibble(cbind(loglike, true_cell_clust)),
       aes(x = V1, y = V2, color = as.factor(true_cell_clust))
      ) +
  geom_point() +
    geom_abline(intercept = 0, slope = 1)+
    labs(x = "loglike for fitting into cluster 1", y = "loglike for fitting into cluster 2")

```


And now that we have our loglike data, it's time to re-assign cluster labels. This is straightforward, we just select the r2 column with the lowest r2, indicating best fit.
...
```{r}
  ...
  updated_cell_clust <-  sapply(1:nrow(loglike), function(row) which.max(loglike[row,]))
...
```
So in this super simple version the cells find their way back. But in more complicated version (ie the neftel 2019) things go haywire.

Addendum: Similar plots with our current understanding of Felix's method:


  ```{r  fig.show="hold", out.width="50%"}

#Felix version would not do as we did above, but would instead , as i understand, do
target_gene_means <- vector('numeric', length = nrow(dat) )
SS_tot <- vector('numeric', length = nrow(dat))

r2_alt <- matrix(0, nrow = nrow(dat), ncol = n_cell_clusters)
for(cell in 1:nrow(dat)){
  #note that this instead calculated the mean target gene expression for the cell
  #and return this as a scalar
  target_gene_means[cell] <- mean(dat[cell, 1:n_target_genes])

  # Here i am not sure what we should do.
  #normal r2 would compare the target gene expression vector
  #predicted by the model to an average
  SS_tot[cell]  <- sum((dat[,1] - target_gene_means[cell])^2)
...
for(cell in 1:nrow(dat)){
  for(cell_cluster in 1:n_cell_clusters){
    ...
    r2_alt[cell,cell_cluster] <- 1-(S_ERR/SS_tot[cell])
  }
}
...
ggplot(data = as.tibble(cbind(r2_alt, true_cell_clust)),
       aes(x = V1, y = V2, color = as.factor(true_cell_clust))
) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1)+
  labs(x = "r2 for fitting into cluster 1", y = "r2 for fitting into cluster 2")

# Update cluster allocations
updated_cell_clust <-  sapply(1:nrow(r2_alt), function(row) which.min(r2_alt[row,]))
...
```
So in this situation the two methods do the same thing.
