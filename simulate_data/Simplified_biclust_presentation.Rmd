...
The point is that a proper biclust can use some other clustering to find initial cluster labels, and then hopefully biclust coult improve this initial clustering using the additional information from scregclust
...
The real thing would obviously go through many iterations, but for now let's just walk through one iteration.
...
colnames(cell_cluster_history) <- c("Cell ID",'true clusters', 'Initial clustering', paste0("Iteration ", 1:max_iter))
...
ggplot(as.data.frame(dat[,]), aes(x = regulator_expression, y = target_expression)) +
  geom_point() +
...
And now that we have our r2 data, it's time to re-assign cluster labels. This is straightforward, we just select which column of r2 has lowest r2, indicating best fit.
Usually this would raise the concern of overfitting, since a cell was used to fit the model for the cluster it was in the start of the iteration. If that was a big problem, cells would tend to be assigned to the same cluster as they started in. As you can see, that's not really a problem.
