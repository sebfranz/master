...
To do this, we need to calculate the predictive r2 for each individual cell, for how well it fits into the model fitted to each cell cluster. 
...
# For all cells, calculate the predicted r2 for all cell clusters.
  ...
  ggplot(data = as.tibble(cbind(r2, true_cell_clust)),
...
```{r  fig.show="hold", out.width="50%"}
 ...
 #Felix version would not do as we did above, but would instead , as i understand, do
  ...
  ggplot(data = as.tibble(cbind(r2, r2_alt)),
         aes(x = V1, y = V3)
       ) + geom_point()
